...
**gstack 就是我现在的工作方式。** 它把 Claude Code 变成一支虚拟工程团队：有 CEO 重新思考产品，有 eng manager 锁定架构，有 designer 挡住 AI slop，有 reviewer 挖出生产事故，有 QA lead 打开真实浏览器，有 security officer 做 OWASP + STRIDE 审计，还有 release engineer 负责把 PR 发出去。二十三位专家，八个增强工具，全部是 slash commands，全部是 Markdown，全部免费，MIT license。
...
对于兼容 Codex 的 hosts，setup 现在同时支持两种安装方式：从 `.agents/skills/gstack` 进行 repo-local install，或从 `~/.codex/skills/gstack` 进行 user-global install。全部 31 个 skills 都能在所有受支持的 agents 上工作。基于 hook 的 safety skills（careful、freeze、guard）在非 Claude hosts 上会回退为内联安全提示文本。
...
| `/design-shotgun` | **Design Explorer** | 生成多个 AI 设计变体，在浏览器里打开对比板，让你一路迭代到确认方向。taste memory 会逐渐偏向你的个人偏好。 |
| `/design-html` | **Design Engineer** | 接收 `/design-shotgun` 批准过的 mockup，并使用 Pretext 生成 production-quality HTML。文本在窗口缩放时会真实 reflow，而不是被写死的高度撑坏。智能 API 路由会按设计类型选择合适的 Pretext patterns。支持 React/Svelte/Vue 框架检测。 |
...
| `/connect-chrome` | **Chrome Controller** —— 启动带 Side Panel extension 的 Chrome。你可以实时观看每一步操作，检查任意元素的 CSS，清理页面并截图。每个 tab 都有自己的 agent。 |
...
**设计在中心位置。** `/design-consultation` 会从零构建设计系统、研究你所在领域、提出值得一试的创意风险，并写出 `DESIGN.md`。`/design-shotgun` 会生成多个视觉变体，并打开一个对比板供你挑选方向。`/design-html` 会把批准的 mockup 转成带 Pretext 的 production-quality HTML，让文本在窗口缩放时真实 reflow，而不是被写死高度撑坏。随后 `/design-review` 和 `/plan-eng-review` 会继续读取你的选择。设计决策会流过整个系统。
...
**想用带命名空间的命令？** `cd ~/.claude/skills/gstack && ./setup --prefix` —— 把 `/qa` 切换为 `/gstack-qa`。如果你会同时使用其他 skill packs，这会更合适。
