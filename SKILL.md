...
version: 1.1.0
description: |
  gstack is a private AI engineering workflow for day-to-day product work. It routes
  requests into focused planning, review, debugging, QA, shipping, documentation, and
  memory workflows so the agent uses a structured skill instead of answering ad hoc.
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
**Routing rule:** When a request clearly matches a core workflow, invoke the matching skill instead of answering ad hoc.

**Default core workflows:**
- New product idea, brainstorming, "is this worth building" → invoke `/office-hours`
- Product scope, ambition, or strategy review → invoke `/plan-ceo-review`
- Architecture, data flow, performance, or test planning → invoke `/plan-eng-review`
- UI or UX plan review before implementation → invoke `/plan-design-review`
- Bug, regression, broken behavior, root cause analysis → invoke `/investigate`
- Code review, diff review, pre-merge review → invoke `/review`
- QA, browser verification, staging validation → invoke `/qa`
- Ship, push, PR creation, release prep → invoke `/ship`
- Independent second opinion or Codex review → invoke `/codex`
- Post-ship documentation sync → invoke `/document-release`
- Past patterns, learned preferences, prior pitfalls → invoke `/learn`
